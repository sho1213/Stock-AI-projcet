...
- `onnxruntime ... Skipping pci_bus_id ...` は環境依存の警告で、CPU実行では無視できるケースが多いです。
- `VAD filter removed ... of audio` は無音区間を除去した正常ログです。
- `large-v3` は高精度ですが処理時間が長くなります。実行中はセグメント進捗ログが間欠的に出力されます。
